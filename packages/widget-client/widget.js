(function () {
  ...
  function checkAndInjectLeadForm(text: string) {
    // If the message asks user for email/leads or thanks them, let's inject a structural lead form
  ...
  // Track event periodically every 15 seconds
  setInterval(trackEvent, 15000);

  // RULE 1: Time on page trigger (Proactive chat)
  setTimeout(() => {
  ...
  document.addEventListener('mouseleave', (e) => {
    if (e.clientY < 20 && !hasTriggeredProactive) { // mouse moved to close or URL bar
      hasTriggeredProactive = true;
